    ...
    public function overall_leaves()
    {
        try {
            ...
            if (emp_department() == 'Software Development' || emp_department() == 'Human Resource') {
                $leaveData = $leaveQuery->paginate(15);
            } else {
                $employeeIDs = array_column(reporting_team(), 'EmployeeID');
                $leaveData = $leaveQuery->whereIn('leave_Requisition.EmployeeID', $employeeIDs)->paginate(15);
            }

            // Available leave types query
            ...
            // Combine both results in one response
            return response()->json([
                'leaveData' => $leaveData,
                'leaveTypes' => $leaveTypes
            ]);
        } catch (QueryException $e) {
            throw new ErrorException("Error getting leaves: " . $e->getMessage());
        }
    }
