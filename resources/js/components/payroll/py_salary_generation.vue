        ...
        <div class="app-content content ">
            ...
            <div class="content-wrapper container-xxl p-0">
                <div class="content-body">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="alert alert-primary top-radius bottom-radius border-0 p-3"
                                    ...
                                    <ul class="nav nav-pills mb-2">
                                        ...
                                        <li v-if="hasPermission('Payroll HR Approval')" class="nav-item">
                                            <router-link to="/payroll/payroll_hr_approval" class="nav-link">
                                                ...
                                                <span class="fw-bold">HR Approval<span v-if="salary_state.hr === true"
                                        ...
                                        <li v-if="hasPermission('Payroll Finance Approval')" class="nav-item">
                                            <router-link to="/payroll/payroll_finance_approval" class="nav-link">
                                                ...
                                                <span class="fw-bold">Finance Approval<span
                                                        v-if="salary_state.fin === true" class="red-dot"></span></span>
                                            </router-link>
                                        </li>
                                        <li v-if="hasPermission('Payroll Salary Distribution')" class="nav-item">
                                            <router-link to="/payroll/distribution" class="nav-link">
                                                <i class="fa-solid fa-money-bill-wave"></i><span
                                                    class="fw-bold">Distribution<span v-if="salary_state.dis === true"
                                                        class="red-dot"></span></span>
                                            </router-link>
                                        </li>
                            ...
                            <div class="card top-radius bottom-radius border-0">
                                <div class="row p-3" style="margin-top:10px">
                                    ...
                                    <div v-if="hasPermission('Payroll Proceed to HR approval')" class="col-md-3">
                                        <button :disabled="disabled1" @click="delay1()" data-bs-toggle="modal"
                                            data-bs-target="#hireinterview" class="btn btn-primary bg-primary">Proceed
                                            for HR Approval </button>
                                    </div>
                                    ...
                                    <table class="table table-hover">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="sticky-th-center"
                                                    title='This deduction will be take place at "Finance Approval" step.'>
                                                    Loan<br>Advance</th>
                                        ...
                                        <tbody>
                                            <tr v-for="all_sals1 in all_sals.data"
                                                :class="[all_sals1.DStatus == 'H' ? 'table-danger' : '']">
                                                <td class="td-center">{{ all_sals1.EmployeeCode }}<br /><span
                                                        :class="[all_sals1.DStatus == 'P' ? 'text-success fw-bold' : '', all_sals1.DStatus == 'H' ? 'text-danger fw-bold' : '']">{{ all_sals1.DStatus }}</span>
                                                </td>
                                                <td class="td_left">
                                                ...
                                                <td class="td-center">
                                                    {{ Math.floor(all_sals1.Deduction).toLocaleString() }}</td>
                                                <td class="td-center">{{ all_sals1.OverTime }}</td>
                                                <td class="td-center">{{ Math.floor(all_sals1.Salary).toLocaleString() }}
                                                </td>
                                                <td class="td-center">{{ Math.floor(all_sals1.DAmount).toLocaleString() }}
                                                </td>
                                                <td class="td-center">{{ Math.floor(all_sals1.OAmount).toLocaleString() }}
                                                </td>
                                                <td class="td-center">{{ Math.floor(all_sals1.TAmount).toLocaleString() }}
                                                </td>
                                                <td title='This deduction will be take place at "Finance Approval" step.'
                                                    class="td-center">
                                                    {{ Math.floor(all_sals1.loanAmount).toLocaleString() }}<br>
                                                    {{ Math.floor(all_sals1.advanceAmount).toLocaleString() }}
                                                </td>
                                                <td class="td-center"
                                                    v-if="hasPermission('Payroll Add deduction and overtime')">
                                                    <a @click="fetch_emp_payroll(all_sals1.SessionReportID)"
                                                        data-bs-toggle="modal" data-bs-target="#editpayroll"><i
                                                            style="color:#d42f2f"
                                ...
                                <div style="text-align:center; padding-top:20px">
                                    <pagination :limit="limit" :data="all_sals" @pagination-change-page="getResults">
                                    </pagination>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="modal-body pb-5 px-sm-5 pt-50">
                                    ...
                                    <form id="editUserForm" class="row gy-1 pt-75" onsubmit="return false">
                                        <div class="col-12 col-md-6">
                                            ...
                                            <input type="text" class="form-control" v-model="m_session_reportID"
                                                hidden />
                                        ...
                                        <div class="col-12 col-md-6">
                                            <label class="form-label" for="modalEditDeduction">Deduction (Days) <span
                                                    style="color: #DB4437; font-size: 11px;">*</span></label>
                                            <input v-if="m_salary_status == 'P'" type="number" class="form-control"
                                                v-model="m_deduction" placeholder="In Days" />
                                            <input v-else readonly type="number" class="form-control"
                                                v-model="m_deduction" placeholder="In Days" />
                                            <span style="color: #DB4437; font-size: 11px;"
                                                v-if="m_deduction == ''">{{ e_m_deduction }}</span>
                                        </div>
                                        <div class="col-md-3">
                                            <label class="form-label" for="modalEditOvertime">Overtime (Minuts)</label>
                                            <input type="number" class="form-control " v-model="m_overtime"
                                                placeholder="In minuts" readonly />
                                        </div>
                                        <div class="col-md-3">
                                            <label class="form-label" for="modalEditOvertime">Overtime (Hours) <span
                                                    style="color: #DB4437; font-size: 11px;">*</span></label>
                                            <input v-if="m_salary_status == 'P'" type="number" class="form-control "
                                                v-model="h_overtime" placeholder="In hours" />
                                            <input v-else readonly type="number" class="form-control"
                                                v-model="h_overtime" />
                                            <span style="color: #DB4437; font-size: 11px;"
                                                v-if="!h_overtime || h_overtime < 0">{{ e_m_overtime }}</span>
                                        </div>
                                        <div class="col-12 col-md-6">
                                        ...
                                        <div class="col-12 text-center mt-2 pt-50">
                                            <button v-if="!m_deduction || !h_overtime || h_overtime < 0"
                                                :disabled="disabled" @click="delay()"
                                ...
                                <div class="modal-body pb-5 px-sm-5 pt-50">
                                    <div class="text-center mb-2">
                                        ...
                                        <div class="text-center">
                                            <button type="button" @click="proceedtohrapproval()"
