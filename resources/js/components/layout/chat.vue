                ...
                <div class="sidebar">
                    ...
                    <div class="sidebar-content">
                            ...
                            <div class="d-flex align-items-center w-100">
                                <div class="sidebar-profile-toggle">
                                    <div class="avatar avatar-border">
                                        <img v-if="this.fetchimage!=''"
                                             v-bind:src="`public/images/profile_images/${this.fetchimage}`"
                                             alt="user_avatar" height="42" width="42"/>
                                        <img v-else src="public/images/profile_images/pro.png" alt="user_avatar"
                                             height="42" width="42"/>
                                ...
                                <div class="input-group input-group-merge ms-1 w-100">
                                    ...
                                    <input v-model="keyword1" type="text" class="form-control round" id="chat-search"
                        ...
                        <div id="users-list" class="chat-user-list-wrapper list-group">
                            <h4 v-if="chatusers.length > 0" class="chat-list-title">Unread Messages</h4>
                            <ul class="chat-users-list chat-list media-list" v-for="chatusers1 in chatusers">
                                <li @click="messages(chatusers1.email)">
                                    <span class="avatar"><img
                                        v-bind:src="`public/images/profile_images/${chatusers1.photo}`" height="42"
                                        width="42" alt="Generic placeholder image"/>
                                        <span class="avatar-status-offline"></span>
                                    </span>
                                    <div class="chat-info flex-grow-1">
                                        <h6 class="mb-0">{{ chatusers1.first_name }} {{ chatusers1.last_name }}</h6>
                                        <p class="card-text text-truncate">
                                            {{ chatusers1.email }}
                                        </p>
                                    </div>
                                    <div class="chat-meta text-nowrap">
                                        <small class="float-end mb-25 chat-time" style="text-align: right;width: 100%;">{{ chatusers1.emp_code }}</small>
                                        <span class="badge bg-danger rounded-pill float-end">{{ chatusers1.num }}</span>
                                    </div>
                                </li>
                            </ul>
                            <h4 class="chat-list-title">All Contacts</h4>
                            <ul class="chat-users-list contact-list media-list" v-for="overallusers1 in overallusers">
                                <li @click="messages(overallusers1.email)">
                                    <span class="avatar">
                                        <img v-bind:src="`public/images/profile_images/${overallusers1.photo}`"
                                             height="42" width="42" alt="Generic placeholder "/>
                                    </span>
                                    <div class="chat-info">
                                        <h6 class="mb-0">{{ overallusers1.first_name }}
                                            {{ overallusers1.last_name }} ({{ overallusers1.emp_code }})</h6>
                                        <p class="card-text text-truncate">
                                            {{ overallusers1.email }}
                                        </p>
                                    </div>
                ...
                <div class="content-wrapper container-xxl p-0">
                    ...
                    <div class="content-body">
                        ...
                        <section class="chat-app-window">
                            <!-- Active Chat -->
                            <div v-if="conID!=''" class="active-chat ">
                                <div v-for="userdetails1 in userdetails">
                                                ...
                                                <div class="d-flex justify-content-left align-items-center">
                                                    <div class="avatar-wrapper">
                                                        <div class="avatar  me-1">
                                                            <img
                                                                v-bind:src="`public/images/profile_images/${userdetails1.photo}`"
                                                    ...
                                                    <div class="d-flex flex-column">
                                                        <a class="user_name text-truncate text-body"><span
                                                            class="fw-bolder">{{ userdetails1.first_name }} {{ userdetails1.last_name }} </span></a><small
                                    ...
                                    <div v-if="message!='No Messages'" class="user-chats"
                                         style="overflow-y: auto;max-height: 450px;min-height:450px">
                                        <div class="chats" v-for="m in message">
                                            <div class="chat" v-if="m.user_from==fetchsession">
                                                <div class="chat-avatar">
                                                <span class="avatar box-shadow-1 cursor-pointer">
                                                    <img v-bind:src="`public/images/profile_images/${fetchimage}`"
                                                ...
                                                <div class="chat-body">
                                                    <div class="chat-content">
                                                        <p>{{ m.message }}</p>
                                                        <small style="font-size: 8px;">{{ m.m_date }}</small>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <div v-else class="start-chat-area"
                                         style="overflow-y: auto;max-height: 430px;min-height:430px">
                                        <div class="mb-1 start-chat-icon">
                                            <i style="font-size:70px !important;margin-left:-15px"
                                               class="fa-solid fa-comment"></i>
                                        </div>
                                        <h4 class="sidebar-toggle start-chat-text">No Messages</h4>
                                    </div>
                                    <div class="chat-app-form">
                                        ...
                                        <span class="speech-to-text input-group-text"><i
                                            ...
                                            <input type="text" v-model="message_input" class="form-control message"
                                                   placeholder="Message"
                            ...
                            <div v-else-if="conID==''" class="start-chat-area"
...
export default {
    ...
    methods: {
        enterChat() {
            axios.post('./submit_messages', {
                message_input: this.message_input,
                user_to: this.user_to,
                user_from: this.fetchsession,
                conid: this.conID,
            })
                .then(
                    response => {
                        this.message = response.data;
                        this.conID = response.data[0].con_id;
                        this.message_input = '';
                    })
        },
        messages(id) {
            axios.post('./conv_messages', {
                id: id,
            })
                .then(
                    response => {
            ...
            axios.post('./get_userdetail', {
                id: id,
            })
                .then(data => {
                    this.userdetails = data.data;
                    this.user_to = data.data[0].email;
                })
        },
        message2(id) {
            axios.post('./conv_messages2', {
                id: id,
            })
                .then(
                    response => {
        ...
        intervalFetchData: function () {
            ...
            const interval1 =setInterval(() => {
                if (this.user_to != '') {
                    this.message2(this.user_to);
                }
            }, 1000);
            this.intervalIDs.push(interval1);

            const interval2 =setInterval(() => {
                axios.get('chat_allusers')
                    .then((response) => this.chatusers = response.data)
            }, 1000);
            this.intervalIDs.push(interval2);
        },
        getResults(){
            axios.get('userlivesearch', { params: { keyword1: this.keyword1 } })
                .then((response) => this.overallusers = response.data)
        }
    },
    ...
    mounted() {
        ...
        axios.get('./fetch_user_session')
            .then((response) => this.fetchsession = response.data)
