                    ...
                    <section class="app-user-list">
                        <div class="card">
                            <div style="margin-bottom:20px;padding-top:20px" class="d-flex justify-content-between align-items-center header-actions mx-2 row mt-75">
                                <div class="col-md-2">
                                    <div  v-if="hasPermission('Purchase Orders Create')" class="dt-buttons d-inline-flex mt-50">
                                        <router-link style="float:left" to="/purchase/create" class="btn btn-primary waves-effect">+ Create PO</router-link>
                                    </div>
                                </div>
                                <div class="col-md-10">
                                    <div class="dt-action-buttons d-flex align-items-center justify-content-center justify-content-lg-end flex-lg-nowrap flex-wrap">
                                        <div class="col-md-2">
                                            <input @change="search_byDate" type="date" v-model="datefrom" class="form-control" />
                                        </div>
                                        <div class="col-md-2">
                                            <input @change="search_byDate" type="date" v-model="dateto" class="form-control" />
                                        </div>
                                        <div class="col-md-1">
                                            <a @click="clear()" style="float:left" class="btn btn-outline-secondary waves-effect">Clear</a>
                                        </div>
                                        <div class="col-md-5">
                                            <input type="text" v-model="keyword1" class="form-control" placeholder="Search By PO Code / Vendor Name / Req. ID" />
                                        </div>
                                        <div class="col-md-2">
                                            <select @change="filter_byStatus()" v-model="status1" class="form-select ms-50 text-capitalize">
                                ...
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        <tr v-for="adsdata1 in adsdata.data">
                                            ...
                                            <td class="td-left">
                                                <div class="d-flex justify-content-left align-items-center">
                                                    <div class="d-flex flex-column">
                                                        <h6 class="user-name text-truncate mb-0">{{adsdata1.vendorName}}</h6>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td class="td-right">{{Math.floor(adsdata1.ShippingCharges).toLocaleString()}}</td>
                                            <td class="td-right">{{Math.floor(adsdata1.TotalAmount).toLocaleString()}}</td>
                                            <td class="td-center">
                                                <span v-if="adsdata1.Status2==='Not Delivered' || adsdata1.Status2==='Reverse'" class="badge badge-light-danger"> {{adsdata1.Status2}} </span>
                                                <span v-else-if="adsdata1.Status2==='Received'" class="badge badge-light-warning"> {{adsdata1.Status2}} </span>
                                                <span v-else-if="adsdata1.Status2==='Issued'" class="badge badge-light-success"> {{adsdata1.Status2}} </span>
                                                <span v-else-if="adsdata1.Status2==='Not Completed'" class="badge badge-light-danger"> {{adsdata1.Status2}} </span>
                                                <span v-else-if="adsdata1.Status2==='Completed'" class="badge badge-light-success"> {{adsdata1.Status2}} </span>
                                            </td>
                                            <td class="td-center">
                                                <div  v-if="hasPermission('Purchase Orders Actions')" class="d-flex align-items-center col-actions">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-end">
                                                        <a target="_blank" v-bind:href="`Accounts/PO_letter/${adsdata1.PurchaseOrderID}/${adsdata1.PoCode}`" class="dropdown-item">
                                                        ...
                                                        <a class="dropdown-item" v-if="adsdata1.Status2==='Not Delivered' || adsdata1.Status2==='Not Completed'" @click="fetch_poid(adsdata1.PurchaseOrderID)" data-bs-toggle="modal" data-bs-target="#payloan1">
                                                            <i class="fa-brands fa-creative-commons-sa"></i>
                                                            PO Reverse
                                                        </a>
                                                    </div>
                            ...
                            <div style="display:flex; justify-content:center; padding-top:20px">
                                <pagination :limit='limit' v-if='pageNo===4' :data="adsdata" @pagination-change-page="getResults"></pagination>
                                <pagination :limit='limit' v-if='pageNo===1' :data="adsdata" @pagination-change-page="getResult"></pagination>
                                <pagination :limit='limit' v-else-if='pageNo===2' :data="adsdata" @pagination-change-page="search_byDate"></pagination>
                                <pagination :limit='limit' v-else-if='pageNo===3' :data="adsdata" @pagination-change-page="filter_byStatus"></pagination>
                            </div>
                        </div>
                    </section>
                    <!-- users list ends -->
                                ...
                                <div class="col-xl-12 col-md-12 col-12">
                                    <div class="card invoice-preview-card" v-for="perorders1 in perorders">
                                        ...
                                        <div class="card-body  pb-0">
                                            <div class="d-flex justify-content-between flex-md-row flex-column invoice-spacing mt-0" style="margin-bottom:0">
                                                <div v-for='companydetail1 in companydetail' style="margin-left:30px">
                                                    <div style="padding-top:10px;margin-bottom: 0;" class="logo-wrapper">
                                                        <h3 class="text-primary invoice-logo" style="margin-left: 0;">{{companydetail1.company_name}}</h3>
                                                    </div>
                                            ...
                                            <table class="table">
                                                ...
                                                <tbody>
                                                    <tr v-for="poProducts1 in poProducts">
                                                        ...
                                                        <td>
                                                            <span class="fw-bold">{{Number(poProducts1.Price).toLocaleString()}}</span>
                                                        </td>
                                                        <td>
                                                            <span class="fw-bold">{{Number(poProducts1.SubTotal).toLocaleString()}}</span>
                                                        </td>
                                                    </tr>
                                                </tbody>
                                            </table>
