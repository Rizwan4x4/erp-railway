                ...
                <div class="noprint content-header row">
                    <div class="breadcrumb-wrapper">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item">
                                <router-link to="../../purchase/return" style="text-decoration: none;">Purchase Returns</router-link>
                            </li>
                            <li class="breadcrumb-item active">New Purchase Return</li>
                        </ol>
                    </div>
                </div>
                        ...
                        <div class="row invoice-add">
                            ...
                            <div class="col-xl-9 col-md-9 col-12">
                                <div class="card invoice-preview-card">
                                    ...
                                    <!-- Header ends -->
                                    <div class="divider">
                                        <div class="divider-text" style="font-size: 24px;font-weight: 900;">Purchase Return</div>
                                    </div>
                                        ...
                                        <div class="row row-bill-to invoice-spacing" style="margin-top:0px">
                                            <div class="col-xl-12 mb-lg-1 col-bill-to ps-0">
                                                <div class="form-group">
                                                    <h6 class="invoice-to-title" style="margin-bottom:5px">Select Invoice: <span style="color: #DB4437; font-size: 11px;">*</span></h6>
                                                    <div class="invoice-customer">
                                                        <multiselect style="margin-right: 10px;" :options="options" @input="get_vendor_detail()" value="id" label="label" v-model="grn_po" placeholder="Select Purchase Invoice"></multiselect>
                                            ...
                                            <div class="col-xl-6 p-0 ps-xl-2 mt-xl-0 mt-2">
                                                <div class="form-group">
                                                    ...
                                                    <div class="invoice-customer">
                                                        <input type="text" v-model="vendor" readonly class="form-control" placeholder='Vendor Name' style="" />
                                                        <input type="text" v-model="poid" class="form-control" hidden style="" />
                                                        <input type="text" hidden v-model="type" class="form-control" style="" />
                                                        <span style="color: #DB4437; font-size: 11px;" v-if="vendor=='' && (grn_po != '' || grn_po != null)">{{e_vendor}}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                                        ...
                                                        <div class="col-12 d-flex product-details-border position-relative pe-0">
                                                            <div class="row w-100 pe-lg-0 pe-1 py-2">
                                                                ...
                                                                <div class="col-lg-2 col-12 my-lg-0 my-2">
                                                                    ...
                                                                    <input style="padding-left:0px;padding-right:0px;text-align:center" name="third[]" readonly type="number" class="form-control" :value='get_po_detail11.RecvdQuantity' />
                                                                </div>
                            ...
                            <div class="col-xl-3 col-md-4 col-12">
                                <div class="card">
                                    <div class="card-body">
                                        <button :disabled="disabled" @click="delay()" class="btn btn-primary bg-primary w-100 mb-75">Post Return</button>
                                    </div>
                                </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            get_vendor_detail() {
                axios.get('accounts/get_purchaseinvoice_detail/' + this.grn_po.id)
                    .then(response => {

                        this.vendor = response.data[0].vendorName;
                        this.poid = response.data[0].ReceavingOrderID;
                        this.type = response.data[0].RequisitionType;
                        this.req_id = response.data[0].ReqID;
                ...
                axios.get('accounts/get_purchaseinvoice_detail1/' + this.grn_po.id)
                    .then(response => {
            ...
            issue_grn() {
                if (this.grn_po == '' || this.grn_po == null || this.vendor == '' || this.invoice_no == '') {
                    if (this.vendor == '' && this.grn_po != '' && this.grn_po != null) {
                        this.e_vendor = 'Invoice don\'t have Vendor Name';
                    }
                    if (this.invoice_no == '') {
                        this.e_invoice_no = 'Enter invoice number';
                    }
                    if (this.grn_po == '' || this.grn_po == null) {
                        this.e_grn_po = 'Please select Purchase Invoice';
                    }
                    ...
                    axios.post('./accounts/insert_purchase_return', {
                        item_name: k,
                        ordrqty: m,
                        qty: n,
                        unit: o,
                        ...
                        .then(data => {
                            if (data.data == "submitted") {
                                this.$toastr.s("New Purchase Return added Successfully", "Congratulations!");
                                this.$router.push({ name: 'purchase_return' })
                            }
                            else {
        ...
        mounted() {
            ...
            axios.get('./accounts/get_POreturn')
                .then(response => {

                    this.get_vendors = response.data;
                    this.options = [];
                    this.options = this.get_vendors.map((inv) => ({
                        id: inv.ReceavingOrderID,
                        label: `${inv.FormID}` + '_Vendor: ' + `${inv.VendorName}`,
                    }));
                })
