...
<li class="breadcrumb-item">
<router-link to="/sales/invoice_detail" style="text-decoration: none;">Invoices Detail</router-link>
</li>
...
<!-- Header ends -->
<div class="divider">
<div class="divider-text" style="font-size: 24px;font-weight: 900;">Sales Invoice</div>
</div>
...
<div class="form-group">
    <h6 class="invoice-to-title" style="margin-bottom:5px">Against Customer:<span style="color: #DB4437; font-size: 11px;">*</span></h6>
    <div class="invoice-customer">
        <select v-model="customer" class="invoiceto form-select">
            <option value=''>Select Customer</option>
            <option v-for='get_customers1 in get_customers' :value='get_customers1.CustomerID'>{{ get_customers1.CustomerName}}</option>
        </select>
        <span style="color: #DB4437; font-size: 11px;" v-if="customer==''">{{e_customer}}</span>
    </div>
</div>
                ...
                <div class="col-12 d-flex product-details-border position-relative pe-0">
                    <div class="row w-100 pe-lg-0 pe-1 py-2">
                        ...
                        <div class="col-lg-2 col-12 my-lg-0 my-2">
                            ...
                            <input @change="sum_total()" style="padding-left:0px; padding-right:0px; text-align:center" name="second[]" type="number" class="form-control" />
                        </div>
                        <div class="col-lg-3 col-12 mt-lg-0 mt-2">
                            <p class="card-text col-title mb-md-50 mb-0">Units</p>
                            <select class="form-select item-details" name="third[]">
                                <option value="">Select</option>
                                <option v-for='get_unit1 in get_unit' :value='get_unit1.UnitName'>{{ get_unit1.UnitName}}</option>
                            </select>
                        </div>
                        <div class="col-lg-2 col-12 mt-lg-0 mt-2">
                            <p class="card-text col-title mb-md-2 mb-0">Unit Cost</p>
                            <input @change="sum_total()" name="fourth[]" type="number" class="form-control" />
                        </div>
                        <div class="col-lg-2 col-12 mt-lg-0 mt-2">
...
<div class="card">
<div class="card-body">
<button :disabled="disabled" @click="delay()" class="btn btn-primary bg-primary w-100 mb-75">Post Invoice</button>
...
this.post_purchase_invoice();
},
post_purchase_invoice() {
if (this.date == '' || this.invoice_no == '' || this.customer == '') {
if (this.date == '') {
...
if (this.invoice_no == '') {
this.e_invoice_no = "Please enter invoice number";
}
else {
this.e_invoice_no = "";
}
if (this.customer == '') {
this.e_customer = "Please select customer";
}
else {
this.e_customer = "";
}
...
var quantity = document.getElementsByName('second[]');
var unit = document.getElementsByName('third[]');
var unit_cost = document.getElementsByName('fourth[]');
var k = 'zero';
var m = 0;
var n = 0;
var o = 0;
...
for (var h = 0; h < unit_cost.length; h++) {
var d = unit_cost[h];
n = n + "|" + d.value;
}
...
axios.post('./accounts/insert_saleinvoice', {
item_name: k,
quantity: m,
unit: o,
unit_cost: n,
...
this.$toastr.s("Sales Invoice Created Successfully", "Congratulations!");
this.$router.push({ name: 'sales_invoice_detail' })
}
else if (data.data == "quantity error") {
this.$toastr.e("Invalid quantity", "Error!");
}
else if (data.data == "Empty field") {
this.$toastr.e("Please fill all fields", "Error!");
}
else {
this.$toastr.e("Sales Invoice Not Created", "Error!");
}
})
}
},
...
axios.get('./accounts/get_customers')
.then(response => this.get_customers = response.data)
